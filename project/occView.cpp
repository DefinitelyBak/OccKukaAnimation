...
void OccView::loadStepFiles(std::string path){
    loadStepFile((path + "/kuka_base_AAA.step").c_str());
...
// Метод принимает путь до step файла в формате С
void OccView::loadStepFile(const char* path){
    // Статическая переменная-счётчик колличества загруженных моделей
    static int countAisLoadedShapes = 0;

    // Инициализация объекта управляющего сичтыванием step файлов
    STEPControl_Reader Reader;

    // Считываение и проверка на корректное открытие step файла
    ...
    // Преобразование загруженного файла в представление OpenCascade и вывод о состоянии преобразования
    qDebug() << Reader.TransferRoots() << " Reader roots transferred.";

    // Создание временного объекта представления 3D модели
    TopoDS_Shape temp_shape = Reader.OneShape();

    // Преобразование матемтаического представления модели в AIS_shape
...
// Метод принимает значения из виджета объекта occQt, отвечающего за запуск анимации и ввода данных
void OccView::slotStartAnim(std::array<double, 8> inputData){

    // Массив передаваемый обхекту класса Animation, для инициализации перемещений обозенных координат и длительности анимации
    ...
    qDebug() << inputData[7];
    // Вызов метода инициализации анимации
